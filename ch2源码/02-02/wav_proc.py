...
class WaveProc(object):
    ...
    def wave_read(wave_path):
        ...
        duration = frame_num / sr       # duration 音频时长
    ...
    def pcm_read(pcm_path, sr):
        """
        pcm文件的读取
        :param pcm_path:文件所在路径
        :param sr:sample rate 采样率
        :return:sr, duration, channel_num, wave_data，采样率，时长，通道数，读出的音频数据
        """
    ...
    def wave_write(wave_data, output_path, sr):
        ...
        :param sr: 采样率      采样率一定要一致，不然声音会很奇怪，被加速或者减慢
        :return: None
        """
...
#
    amp_half_data = data*0.5    # 将语音的幅值降低一半，但是音量不会降低一般，音量和幅值并不是线性的关系
